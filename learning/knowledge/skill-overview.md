...
> 29 个技能（8 自建 + 21 插件），按功能域分为 6 大类。每个技能标注触发关键词和典型场景。
...
| 技能             | 来源  | 触发关键词               | 典型场景                                             |
| -------------- | --- | ------------------- | ------------------------------------------------ |
| **dwg**        | 自建  | DWG转DXF、CAD转换、图层列表 | DWG↔DXF 转换、文字提取/翻译、图层管理、SVG 导出、批量处理               |
| **dxf-review** | 自建  | 视觉复查、预览、DXF检查、对比原图 | DXF 视觉复查：渲染预览、多模态对比、自动验证（尺寸/图层/闭合性） |

### 典型串联

```
DWG 图纸 → dwg 提取中文 → 翻译 → 导出新 DXF → dxf-review 视觉复查
```
...
| 技能         | 来源  | 触发关键词                  | 典型场景                                   |
| ---------- | --- | ---------------------- | -------------------------------------- |
| **pdf2zh** | 自建  | PDF翻译、pdf2zh         | PDF 翻译（保留排版），23+ 翻译引擎，内置 MiMo 翻译补丁   |
