---
name: local-ai
description: 本机本地模型层（llama.cpp CUDA + RTX 5060 Laptop 8GB）—— 省 token、可离线、保隐私。 用途：① 单次轻量任务（改写/摘要/分类/抽取/短句翻译）；② 批量并发处理几十上百条同质小任务； ③ 本地 agent（pi CLI + 本地模型，自己读文件、改写、写回、多步闭环）。 遇到「大量简单重复的活」——批量改写、逐条分类打标、逐条抽取字段、批量摘要、批量翻译短句—— 不要在主上下文里逐条做，也别急着派联网 subagent（那同样烧 token）：交给本机跑完， 只把一份回执（条数 / 异常清单 / 抽样）带回本对话，主模型整体看过再决定落地。 需要读写文件或多步闭环的，交给 pi 在本地跑完，主模型只看摘要。 用户提到"本地处理 / 离线 / 断网 / 不耗 token / 省 token / 最简单任务 / 省电 / 隐私 / 本机模型"时同样使用本技能。 不适用：需要准确事实知识、需要跨条全局推理、或本地小模型明显扛不住的高难度任务。 视觉/OCR/音频默认走本机多模态（vl4/vl8/asr/ocr，0 token、不出本机），视频与高难度视觉推理回退 mimo；但单张图要跟本对话上下文一起推理、且不敏感时，主模型自己就能看，不必起本地模型。
compatibility: |-
...
**一条例外（2026-09-12 起）**：**主模型自己也能看图** —— 单张图、要跟本对话上下文一起推理、
且不敏感时，直接把它 `Read` 给主模型看，别绕本地：质量更高，还免去起 server / 占显存 / 切模型。
代价是图进上下文**计 token**，且**出本机**（所以敏感资料不适用）。
...
**主对话是外发的** —— 它走 `ANTHROPIC_BASE_URL` 那个代理端点。所以「本地跑」只解决了一半：
**只要产物被读进对话，它就已经离开本机了。**

敏感材料走本地流水线时，**给 `llama_batch.py` 加 `--redact`** —— 回执只回「条数 / 标签 / 计数」，
**模型正文与路径一律不回显**：

```bash
py -3 "...\llama_batch.py" 转写.jsonl -o out.jsonl -j 1 --no-think --redact
```

`--redact` 挡的就是这条通道（**不加它，默认回执就会把正文带回对话**）：

| 默认回执里会外发的东西 | 加了 `--redact` |
| --- | --- |
| 「## 抽样」段 —— 首/中/尾 + 第一条异常的**模型原文**（各 ≤120 字符） | 整段省略，只留一行说明 |
| 异常清单的**摘要列** —— `result` / `error` 的文本 | 只留标签（`空结果` / `原样回显` / `失败`…） |
| `- 输入：{路径}` / `- 输出：{路径}`（文件名常含「张三-身份证」这类信息） | 路径隐去 |
| stderr 进度行尾巴的 `→ <out.jsonl>` / `[回执] <路径>` | 同样隐去（Bash 输出也进对话） |

**它只改「回显」，不改产物**：全量结果、异常原因照常落在本地输出文件里。真要抽查正文，
就在**本地**读那份文件，别 `Read` 进本对话；要几条样本进对话，就先去掉 `--redact` 只跑那几条。

实测踩过两次，**两次都不是「调了 mimo」**：

| 踩法 | 后果 |
| --- | --- |
| 直接 `Glob` 列目录 | 60+ 个文件名（含姓名、证件类型）进了对话 |
| 读进程 stdout 只为拿计数 | `model.infer()` 自己把逐行识别正文 print 到 stdout，**证件正文**跟着进了对话 |

推论：**凡是「读进程输出」的地方，先想一遍它会不会带出正文** —— 脚本自己的 stdout 同样受此约束。
（`ocr/run.sh` 的 stdout 已经吞掉了 infer 的那一层，见 `ocr.py:_run_one`；
`ocr.report.md` 天生不含正文，见第十节。）
...
> **但先判断一件事：这张图非本地不可吗？** 主模型（Claude）本身已能看图（2026-09-12 实测通过）。
> **单张图 + 要跟本对话上下文一起推理 + 不敏感** → 直接把它交给主模型读，**不必起本地模型**：
> 零准备、不占显存，也不受「一次只跑一个模型」约束（本地正跑着批量也不用停）。
> ⚠️ 模型能力远大于 4B/8B，但**没做过同批图的对照实测** —— 「比 `vl4` 准」是推断，不是结论。
> 反之，**量大 / 逐条几十张 / 敏感 / 要严格 0 token**，才落到下面这条本地链。
...
> ⚠️ **改这几个模型的文件名前，先看 `evals/evals.json` 的 id 3**：那里的断言把
> `Qwen3VL-4B-Instruct-Q4_K_M.gguf` 这个 **basename 写死了** —— 回执里的 server 模型名取的
> 就是 GGUF 的 basename，而它是 4B / 8B 唯一的机械判别器（见「四、回流」）。
> `start.sh` 的 `MODEL_VL4` 常量旁有同一句提示。改名而不改断言 = eval **静默退化**。

### `vl4` 还是 `vl8`

**默认 `vl4`** —— 快，且比 8B 省约 0.74 GB 显存（实测整卡 6956 vs 7711 MiB，差 755 MiB）。

但 `vl4` 在**细粒度判别**上不可靠：实测同一张图**换个问法结论就翻** —— 一张 2481×1754 的
横版图（不可能是手机截图的比例），问「属于哪一类」答「证件」，问「是不是手机截图」答「是」；
另一张同样从「证件」翻成「是」。8B 换上去之后，同一批 8 张图给出一致答案。

所以下面这类问题**直接上 `vl8`**，别拿 `vl4` 试：

- **二值 / 闭集判别** —— 是不是某种界面、有没有某个章、图里有几个某物
- **区分「像但不同」的类别** —— 公章 vs 电子章、截图 vs 拍照
- **结论要被当作事实用**（比如据此决定「这份材料要不要补交」）

**开放式描述**（「图里有什么」「转成 markdown」）`vl4` 够用，也不值得为它切模型。
...
### 什么时候不用本地：直接给主模型看（2026-09-12 起）

满足这三条就**别绕本地**，把图 `Read` 给主模型即可：

- **单张（或少数几张）** —— 不是几十上百张要逐条跑
- **要跟本对话上下文一起推理** —— 例如「这张报错截图，对应我刚贴的哪段代码」
- **不敏感** —— 图片会上传，见下条

好处是**零准备** —— 不用 `start.sh`、不占显存、不用等 2–4s 加载，本地正跑着 2B/9B 批量也不必停；
能力上大概率强过 `vl4`/`vl8`（4B 连细粒度判别都会翻车，见上），但**这一条是推断、未做对照实测**。

代价：图片进主上下文**计 token**，且**出本机**。
...
- ⚠️ **「给主模型看」那条路同样排除敏感资料** —— 主模型在云端，图片给它看就是**出本机**，
  与送 mimo 是同一类风险。敏感图一律走 `vl4` / `vl8` / `ocr`。
...
> ⚠️ **扫描件/证件/表格类文档，动手前先读
> [`references/ocr-pitfalls.md`](references/ocr-pitfalls.md)** —— 那是 2026-09-13
> 实跑 13 份入职材料（22 页扫描 + 14 页文字层）踩出来的 11 条坑。最关键的三条：
> ① 多页 Base 模式把整页压进 1024 方框，A4 有效只有 **~88 DPI**，小字会成片丢，
> 而 `--dpi` 调多大都没用；② `--image-size` **超过 1024 会崩**（甚至输出 0 字符/循环卡死）；
> ③ **回执写「成功」不等于内容可用** —— 模型会陷入不终止的循环，产物是重复垃圾。
> 真值请靠**校验位**（身份证 GB 11643 / 银行卡 Luhn / 护照 MRZ 三个校验位）与
> **跨文件印证**，不要靠多读几遍。

- ⚠️ **`vl8` 显存余量仅约 440 MiB**（实测整卡 7711 / 8151 MiB）：它是备用而非默认 —— 平时用 `vl4`，只在 4B 明显不够时切过来。溢出时把 ctx 降到 4K。
- ⚠️ **OCR 贴边**（权重实测 6,363 MiB / 门槛 6,600 MiB 空闲）：跑前务必 `stop.sh`，别同时开占显存的程序。卡死残留的 `ocr.py` 进程会在外层任务报完成后继续攥着显存，排查见 `references/ocr-pitfalls.md` §11。
- ✅ **OCR 回执天生不含正文**：`ocr.report.md` 只有计数（总 / 成功 / 失败 / 空结果）、
  页级错误与输出文件名，**没有「抽样」段** —— 识别正文只落各页输出目录的 `result.md`，
  不经回执进主对话。这正是敏感扫描件该有的形态。（`llama_batch.py` 的回执**默认含抽样**，
  所以媒体/转写类敏感批量要加 `--redact`，见「四、回流 · 出口把关」。）
...
- **文本模型的边界**：MiniCPM5-2B 与 Qwen3.8-9B 无 mmproj，均**纯文本**；
  ...
  （例外：单张图要跟对话上下文一起推理、且不敏感 → 主模型自己就能看，见该节开头）。
