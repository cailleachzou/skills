...
description: >
  ...
  通用文本问答/改写/摘要用 llama.cpp Vulkan 后端 + GTX 1080 Ti GPU（翻译统一走 docs-translate，本技能不负责翻译）。
...
compatibility: |
  ...
  Ollama 模型 (GGUF, 存储在 ~/.ollama/models/blobs/):
  ...
  独立 GGUF (直接加载, 无需 ollama):
  - qwen2.5:14b: Qwen2.5 14B Q4_K_M (8.4GB, `C:\Users\caill\models\`)
  - qwen2.5:7b-q6: Qwen2.5 7B Q6_K (5.9GB, 高精度版, `C:\Users\caill\models\`)
...
**不走本地**：需强推理/长上下文/多步 → 主模型；涉及看/听/说 → mimo；复杂文档解析 → `docling` skill；翻译（含一句话/文档翻译）→ `docs-translate` skill。

## 工具总览

| 模型/工具                                | 设备          | 职责                 | 实测速度            |
| ------------------------------------ | ----------- | ------------------ | --------------- |
| llama-server Vulkan + qwen2.5:7b     | GTX 1080 Ti | **中文任务**（改写/问答/摘要） | ~61 tok/s       |
| llama-server Vulkan + qwen2.5:14b    | GTX 1080 Ti | 复杂推理/长文中文          | ~28 tok/s        |
| llama-server Vulkan + qwen2.5:7b-q6  | GTX 1080 Ti | 高精度中文              | ~49 tok/s        |
...
| qwen2.5:14b | `C:\Users\caill\models\Qwen2.5-14B-Instruct-Q4_K_M.gguf` | 复杂推理/长文（-c 2048 可全量进 GPU） |
| qwen2.5:7b-q6 | `C:\Users\caill\models\Qwen2.5-7B-Instruct-Q6_K.gguf` | 高精度中文 |
...
| ------------- | ----- | ----- | ----- | ---------- | ---------- |
| qwen2.5:14b   | 8.4GB | ★★★★★ | ★★★★★ | ~28 tok/s  | ~1.5 tok/s |
| qwen2.5:7b    | 4.4GB | ★★★★★ | ★★★★  | ~61 tok/s  | ~3.6 tok/s |
| qwen2.5:7b-q6 | 5.9GB | ★★★★★ | ★★★★  | ~49 tok/s  | ~3 tok/s   |
...
- **VRAM**: GTX 1080 Ti 11GB，7B 模型约占 5.7GB；qwen2.5:14b (8.4GB) 用 -c 2048 时约 9.5GB 可全量进 GPU，别开大上下文
- **首次加载**: 实测冷启动 qwen7b/qwen7b-q6 约 8.3-8.4s，qwen14b 约 11.2s；强杀关闭约 1.5-2.3s；后续请求快速
- **独立 GGUF**: `C:\Users\caill\models\` 直接用 `-m "绝对路径"` 加载，无需 ollama
