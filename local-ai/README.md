...
**`kv_unified` 意味着 4 个 slot 从同一个 KV 池里取**，不是各拿一份：

- 9B（`-c 32768`）：看着像 4 × 32K，实际整池 32K，**4 并发时每条平均只有 ~8K**。
- 8GB 装不下「4 × 32K」的 KV（那要 6.5GB）—— 整卡实测 6891 MiB（含权重 5502 MiB）就是反证。
- 2B（`-c 131072`）同理：池子 128K，4 并发时每条平均 ~32K。

**所以：批量 + 长 prompt 本来就该走 2B。那不是优化，是 8GB 卡上的唯一可行解。**
